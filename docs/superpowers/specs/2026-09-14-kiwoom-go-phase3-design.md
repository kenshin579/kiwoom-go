...
주의: 스펙은 `item`·`type` 을 depth-1 `String` 으로 적었지만 **실제로는 배열**이다(§3).
...
`usa20290` 은 이름과 달리 응답의 **필드 표**에 실시간 푸시 절이 없다. 푸시는 실제로 오고,
그 FID 는 같은 문서의 `response_example` 에 있다. §8 참고.

미국 쪽은 `trnm` 에 **`G` 접두어**가 붙는다. 스펙에 미국 `조건검색 요청 일반` 의 `trnm` 이
`CNSRREQ` 와 `GCNSRREQ` 두 가지로 적힌 자리가 있는데, 공식 예제가 **`GCNSRREQ`** 로 확정해 준다.
...
한글명이 갈리는 FID 가 **67개** 있다(실시간 398개 안에서는 66개이고, 조건검색까지
더한 400개 기준이 67개다 — 표는 400개를 덮으므로 67이 맞는 수다). 전부 표기 차이다
(`매도1호가` vs `매도호가1`, `시간` vs `체결시간`, `9001` 이 `종목코드`/`종목,업종코드`/
`종목코드,업종코드`). 의미가 실제로 충돌하는 FID 는 없다. FID 당 이름 하나로 접고,
접었다는 사실과 버린 표기를 주석에 남긴다.
...
## 8. 조건검색 — `ka10173`·`usa20290` 둘을 손으로 쓴다

6개(`ka10171`·`ka10172`·`ka10174` 와 미국 짝)는 같은 소켓 위의 요청/응답이라 REST 처럼
생성한다. 패키지는 `domestic/condition`·`overseas/condition`. 나머지 둘은 손으로 쓰는데,
**빼는 이유가 서로 다르다** — `ka10173` 은 생성기가 접지 못해서고, `usa20290` 은 생성기가
읽는 필드 표에 푸시 절이 없어서다(아래 정정 참고).

`ka10173` 은 응답이 **두 벌**이다 — 조회 결과가 한 번 오고, 그 뒤로 `REAL`
푸시가 계속 온다. 스펙에서도 이 API 만 `is_section` 으로 본문이 갈라진다
(`조회 데이터` / `실시간 데이터`).

### `usa20290` — 푸시 FID 는 예제에 다 있다 (2026-09-15 정정)

**이 절의 원래 결론은 틀렸다.** 아래가 원문의 판단이었다:

> `usa20290` 은 푸시 FID 이름표가 스펙에 없으니 조회 응답만 타입을 붙이고 푸시는 `Raw`
> 맵으로만 낸다. FID 이름을 지어내지 않는다.

전제가 틀렸으므로 결론도 틀렸다. **비어 있는 것은 `response.body`(필드 표)이지 스펙 전체가
아니다.** 같은 API 의 `response_example` 에 푸시가 그대로 실려 있다:
...
- 미국 짝 `usa20290` 은 응답 **필드 표**가 조회 한 벌뿐이다. `is_section` 도 없고 `REAL`
  푸시 절도 없다. 여기까지는 원문이 맞다.
- **그러나 `response_example` 에 푸시가 있다.** FID 는 `20`·`841`·`843`·`907`·`9001` 다섯 —
  국내 짝 `ka10173` 과 **똑같은 다섯**이고, 다섯 개 전부 `tools/gen/fids.go` 의 표에 이름이
  있다. 즉 이름을 **지어내는 것이 아니라 이미 가진 이름을 쓰는 것**이다.
- 공식 예제의 `COLUMNS = {}` 는 "FID 를 모른다" 가 아니라 예제 스크립트가 표를 안 채웠다는
  뜻이었다. 그것을 스펙의 침묵으로 읽은 것이 이 절의 실수다.
- 예제가 필드 표를 이기는 것은 **이 저장소의 선례**다. `tools/spec/SOURCE.md` 의
  "스펙 표와 어긋나는 곳" 이 이미 세 번 같은 판단을 했다(REG 의 `item`·`type` 이 배열,
  `values` 가 맵, `trnm` 이 `GCNSRREQ`). `usa20290` 은 그 **네 번째**다.

그래서 이렇게 낸다: 조회 응답도 푸시도 **타입을 붙여** 낸다 — 국내 짝과 대칭이다. 값 필드
이름은 `ka10173` 과 글자까지 같아야 한다(같은 FID 는 어디서나 같은 낱말이다). 받은 FID 는
`Raw` 에도 그대로 남는다.

푸시 봉투의 `stexTp` 는 `values` **밖**에 붙는 필드라 값 구조체에도 `Raw` 에도 자리가 없다.
버리면 미국 종목의 거래소 구분을 어떤 경로로도 얻을 수 없으므로 — 조회 응답에서는 철자가
갈리는 것까지 받아 내는 값이다 — 봉투 타입(`stream.Event.StexTp`)에 자리를 만든다.
`Symbol`·`Name` 과 같은 층이다. 실시간 23종에는 없는 필드라 그쪽에서는 늘 빈 문자열이다.

`usa20290` 을 여전히 **손으로 쓰는** 이유는 남는다. 생성기는 필드 표만 읽는데 푸시 절이
거기 없다 — 337개 중 하나를 위해 생성기에 예제 파서를 넣을 일은 아니다.
...
| ~~`usa20290` 의 푸시 FID 가 스펙에 없다~~ | **해소됨(전제가 틀렸다)** — 필드 표에 없을 뿐 `response_example` 에 다 있다. 조회도 푸시도 타입을 붙인다(§8) |
...
5. 조건검색 6개 생성 + `ka10173`·`usa20290` 손으로(§8)
6. README·검증
