...
**구현 1번 과제로 둔다.** 스펙과 **같은 커밋**(`234560d`)의 공식 예제(`examples/**/*.py`,
`_async`·`_pubsub` 두 스타일)를 받아 확인하고, 알아낸 규약을 `tools/spec/SOURCE.md` 에
근거와 함께 적는다. 기억이나 추측으로 `{"trnm":"LOGIN","token":…}` 라고 쓰지 않는다.

**여기서 막히면 3단계 전체가 막힌다.** 그래서 첫 과제다.
...
## 7. 값 컨테이너가 맵인지 배열인지 — 스펙이 스스로 모순이다

같은 `values` 필드가 실시간 23곳에서는 `LIST`, `ka10173` 에서는 `Object` 로 선언돼 있다.

자식이 FID 키(`"10"`, `"20"`)인 이상 **맵(`Object`)만 말이 된다.** 배열이라면 키가 아니라
인덱스여야 한다. 따라서 `LIST` 쪽이 문서 오류로 **보이지만**, 이것은 가정이다.

**예제와 모의투자 실메시지로 확정한 뒤** 생성기를 쓴다. 핸드셰이크 확인과 같은 과제에
묶는다 — 둘 다 "실제 메시지를 한 번 봐야 알 수 있는 것"이다.
...
### `usa20290` 은 짝이 아니다 — 스펙이 덜 적혔다

미국 짝인 `usa20290` 은 이름이 "미국주식 조건검색 요청 **실시간**"인데 응답이 **조회 한 벌뿐**이다.
`is_section` 도 없고 `REAL` 푸시 절도 없다. `ka10173` 에 있는 `type`·`name`·`values`·FID 가
**통째로 빠져 있다**(대신 `stexTp` 가 하나 더 있다).

API 이름이 실시간을 약속하는데 문서에 푸시 스키마가 없다 — **문서가 덜 적힌 것**으로 본다.
`ka10173` 과 같은 모양일 가능성이 높지만 **그것은 추측이다.**

다루는 법: `ka10173` 은 설계대로 요청/응답 + 채널로 낸다. `usa20290` 은 **실메시지로 확인한
뒤에** 정한다 — 푸시가 `ka10173` 과 같은 모양이면 같게 내고, 확인하지 못하면 **조회 부분만**
내되 "이 API 의 실시간 푸시는 스펙에 없어 지원하지 않는다"를 doc 주석에 **명시한다.**
이름만 보고 실시간이 되는 줄 알았다가 아무것도 안 오는 것이 제일 나쁘다.
...
| 위험 | 다루는 법 |
|---|---|
| **핸드셰이크 미확인** — 막히면 3단계 전체가 막힌다 | 구현 1번 과제. 공식 예제를 벤더링해 확인 |
| **`values` 가 맵인지 배열인지** — 스펙이 자기모순 | 같은 과제에 묶어 실메시지로 확정 |
| **FID 400개 이름을 지어낸다** | 한글명을 모든 줄에 주석으로. 검토를 구현과 분리 |
| **모의투자가 실시간을 주는지 미확인** | 안 주면 가짜 서버 테스트가 유일한 그물이 된다. 그래서 가짜 서버를 얇게 만들지 않는다 |
| **미국 `trnm` 의 `G` 접두어 문서 불일치** | 실메시지로 확정. 확정 전에는 생성하지 않는다 |
| **`usa20290` 의 실시간 푸시가 스펙에 없다** | 실메시지로 확인. 못 하면 조회만 내고 doc 주석에 명시(§8) |

## 12. 커밋 분리

1. 핸드셰이크·`values` 모양 확인과 `SOURCE.md` 기록
