...
이 버전이 보증하는 것과 보증하지 않는 것을 구분해 둔다.

**기계적으로 보증한다.** 337개 중 335개(REST 304 · 실시간 23 · 조건검색 8)에 대해 스펙의
요청/응답 필드가 Go 구조체로 정확히 옮겨졌고, 전체가 `go build`/`go vet`/`go test -race` 를
통과한다. 생성물이 스펙과 바이트까지 같은지는 골든 테스트가 계속 지킨다.
WebSocket 연결·로그인·PING·구독 라우팅·재연결·구멍 신호는 가짜 서버로 검증했다.

**실서버로 확인한 것**(통합 테스트, `-tags integration`):

- 업무 오류가 `HTTP 200` + `return_code != 0` 로 오는 것과, 그것을 에러로 갈라내는 것
- 연속조회(`cont-yn`/`next-key`) 헤더 왕복이 실제로 이어지는 것
- WebSocket 이 운영 서버에서 연결·`LOGIN`·`REG` 까지 통하는 것
- 인증이 계좌 API 까지 통하는 것

**아직 확인하지 않은 것.** 확인한 것은 **경로**이지 335개 각각이 아니다. 필드 이름·타입
(전부 string)·경로는 스펙에서 기계적으로 뽑은 것이고, 실시간 필드 이름은 거기에 더해 손으로
적은 표를 거쳤다. **실시간 푸시의 실제 내용은 아직 보지 못했다**(장 시간에만 온다) — 표에 없는
FID 가 오는지, `values` 에 숫자 값이 섞이는지가 거기 걸려 있다. 스펙 자체의 오류는 이미 네 건
찾아 `tools/spec/SOURCE.md` 에 적었지만, 더 없다고 볼 근거는 없다.

**실거래에 쓰기 전에 그 API 를 직접 호출해 확인할 것.**
