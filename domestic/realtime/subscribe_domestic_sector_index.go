...
// DomesticSectorIndex 는 업종지수(0J) 의 실시간 값이다.
//
// 필드 이름은 tools/gen/fids.go 의 표에서 온다 — 스펙은 FID 숫자와 한글명만 준다.
type DomesticSectorIndex struct {
	// 체결시간 (HHmmss)
...
// SubscribeDomesticSectorIndex 은 업종지수(0J) 을 구독한다.
//
...
func (c *Client) SubscribeDomesticSectorIndex(ctx context.Context, items ...string) (<-chan stream.Event[DomesticSectorIndex], error) {
	src, err := c.ws.Subscribe(ctx, "0J", items, defaultBuffer)
	if err != nil {
		return nil, err
	}
	out := make(chan stream.Event[DomesticSectorIndex], defaultBuffer)
	go func() {
		defer close(out)
		for d := range src {
			out <- decodeDomesticSectorIndex(d)
		}
	}()
	return out, nil
}

func decodeDomesticSectorIndex(d wstransport.Delivery) stream.Event[DomesticSectorIndex] {
	// 봉투는 봉투째 옮긴다. StexTp 는 실시간 23종에는 없어 늘 빈 문자열이지만, 한 곳에서만
	// 옮기고 다른 곳에서 빠뜨리면 키움이 이 필드를 더하는 날 조용히 사라진다.
	ev := stream.Event[DomesticSectorIndex]{Symbol: d.Item, Name: d.Name, StexTp: d.StexTp, Time: d.Time, Err: d.Err}
	if d.Err != nil {
		return ev
	}
